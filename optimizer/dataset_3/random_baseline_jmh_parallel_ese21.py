...
class ConfigCounter:
    ...
    def __next__(self):
        ...
        if self.current == 0:
            while True:
                a = rng.integers(1,self.fork)
                b = rng.integers(1,self.it)
                if a*b >= 3:
                    break
            return (a,b)
        elif self.current == 1:
            return (self.fork,self.it)
        raise StopIteration

data_path = "laaber_preprocessed_ese21_correct_files"
...
for file in files:
    ...
    for inst_funcs in list_of_instability_funcs:
        ...
        full_result_path = "laaber_random_baseline_ese21/"+proj_name+"/"+method_name
